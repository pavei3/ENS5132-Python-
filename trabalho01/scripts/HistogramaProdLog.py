# -*- coding: utf-8 -*-
"""
Created on Tue May  6 11:42:35 2025
...
df = pd.read_csv(r"C:\PythonBPG\Projetos\ENS5132-Python-\trabalho01\data\df_bebidas_cnpj_agg.csv")
...
df = df[df['Tipo'].isin(['Destilados', 'Vinho', 'Cerveja e Chope'])]
...
def criar_histograma(data, tipo, cor, num_bins=5):
    ...
    min_val = data['volume_hl'].min()
    max_val = data['volume_hl'].max()
    ...
    plt.title(f'Distribuição de Produção - {tipo}', fontsize=14)
    plt.xlabel('Produção (hL) - Escala Log', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(f'histogramaAgg_{tipo.lower().replace(" ", "_")}.png', dpi=300, bbox_inches='tight')
    plt.show()
